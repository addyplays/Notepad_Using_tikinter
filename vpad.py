...
from turtle import color, pd, width
...
main_application.title('Vpad test editor')
...
def find_func(event=None):
    ...
    text_replace_label.grid(row=1,column=1,padx=4,pady=4)
    # entry grid
    find_input.grid(row=0,column=1,padx=4,pady=4)
    replace_input.grid(row=1,column=0,padx=4,pady=4)
    ## button grid
...
edit.add_command(label='Paste',image=copy_icon,compound=tk.LEFT,accelerator='Ctrl+V',command=lambda:text_editor.event_generate("<Control v>"))
...
edit.add_command(label='Clear_all',image=copy_icon,compound=tk.LEFT,accelerator='Ctrl+Alt+X',command=lambda:text_editor.delete(1.0,tk.END))
